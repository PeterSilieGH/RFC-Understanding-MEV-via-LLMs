...
export function startTunnel(config: TunnelConfig): Tunnel {
  ...
  const sshArgs = [
    "-NT",
    "-o",
    `ServerAliveInterval=${config.keepalive ?? 30}`,
    "-o",
    ...
    "-o",
    `LocalForward=localhost:${localPort} localhost:${config.remotePort}`,
    config.sshHost,
  ];
...
export function readRpcFromEnv(envPath = resolve(process.cwd(), ".env")): {
  http?: string;
  ws?: string;
} {
  try {
