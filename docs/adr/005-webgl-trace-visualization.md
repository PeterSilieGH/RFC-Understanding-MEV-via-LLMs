...
- The DiscoUI backend (`l2b ui`) runs unmodified from the submodule via its own `Dockerfile.disco-ui`; nginx routes `/api/traces` to our `trace-api` and the rest of `/api` to it. *(Amended 2026-07-09: the compose command override drops the image's baked-in `--readonly` so project creation, terminal `l2b discover`, and disk sources work from `apps/disco`; the image's own protocolbeat build is masked with a tmpfs so port 2021 is API-only, and the discovery projects/cache dirs are bind-mounted from the submodule checkout for persistence.)*
...
- `apps/trace-web` remains in-tree as the lightweight standalone viewer (M2 deliverable) but is no longer part of the compose stack (retired 2026-07-09); the DiscoUI trace panel is the primary visualization going forward.
