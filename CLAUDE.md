...
- `mev-monitor/mev-inspect-py/` — patched Flashbots inspector (Python). Consumed **strictly as the docker image `mev-inspect-py:local`** (built via the compose `tools` profile from this local checkout): a one-off container per block writes classified traces/swaps/arbitrages/sandwiches/liquidations to Postgres. Not a long-running service, and the only runtime dependency on the mev-monitor checkout.
...
Trace visualization lives in `apps/disco` — a clone of l2beat's protocolbeat (pinned commit, vendored `@l2beat/*` shims in `src/vendor/`, deliberate edits marked `DIVERGENCE(mev)`) with an added `trace` panel that reuses the nodes-tab graph stack via a factory-ized store (ADR-005). Its backend is `l2b ui` built from the submodule (`disco-api`, port 2021), run **without** `--readonly` via a compose command override (enables project creation, `l2b discover` from the terminal panel, and sources from disk) and API-only (the image's own protocolbeat build is masked by a tmpfs; discovery projects/cache are bind-mounted from `l2beat/packages/config/`, so UI-created projects land on the host as untracked submodule files). nginx routes `/api/traces` + `/api/contracts` to our `trace-api` (port 2022) and `/api/mev` to `explorer-api` — the trace panel overlays the explorer's per-tx MEV facts (`GET /api/mev/tx/:hash`, joined onto graph nodes via `swaps.trace_address`) and deep-links as `/ui/trace/:txHash` from every explorer transaction (ADR-007). Keep clone edits minimal and marked — re-porting against a newer submodule pin must stay reviewable. Addresses need normalization: l2beat uses `eth:0x…` chain-specific addresses, raw traces use plain `0x…`.
