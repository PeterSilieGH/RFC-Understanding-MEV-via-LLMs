...
test.describe("explorer-api", () => {
  ...
  test("backfill round-trip over an already-inspected block", async ({ request }) => {
    ...
    expect(res.ok()).toBe(true);
    await expect
      .poll(async () => (await (await request.get(`${EXPLORER_API}/api/backfill`)).json()).running, {
        timeout: 15_000,
      })
      .toBe(false);
  ...
  test("analyzed-ranges compresses coverage and covers inspected blocks", async ({ request }) => {
    ...
    const res = await request.get(`${EXPLORER_API}/api/analyzed-ranges?from=${block - 50}&to=${block}`);
    expect(res.ok()).toBe(true);
    const body = await res.json();
    ...
    expect(body.ranges.some((r: { start: number; end: number }) => r.start <= block && block <= r.end)).toBe(true);
  ...
  test("mev-activity aggregates per-block counts", async ({ request }) => {
    ...
    const res = await request.get(`${EXPLORER_API}/api/mev-activity?from=${block - 5000}&to=${block}`);
    expect(res.ok()).toBe(true);
    const body = await res.json();
    expect(Array.isArray(body.blocks)).toBe(true);
...
test.describe("explorer-web", () => {
  ...
  test("loads an inspected block end-to-end through the /api proxy", async ({ page }) => {
    ...
    await expect(page.locator("#currentBlock")).toHaveText(`#${block}`);
  ...
  test("header has three zones with the follow + MEV-only icon toggles (E1/E2)", async ({
    page,
  }) => {
    await page.goto(EXPLORER_WEB);
    const subtitle = page.locator(".header-top .subtitle");
    const h1 = page.locator(".header-top h1");
    ...
    await expect(h1).toHaveText("MEV Block Explorer");
    ...
    // subtitle left of h1, h1 left of the toggle group;
    // follow-latest left of MEV-only left of EUR
    const [s, h, l, m, e] = await Promise.all(
      [subtitle, h1, liveToggle, mevToggle, page.locator("#eurToggle")].map((loc) =>
  ...
  test("stats bar, income tab, MEV-only filter, toast under header (E2-E5)", async ({
    page,
  }) => {
    const block = latestInspectedBlock();
    test.skip(block === null, "no inspected blocks in the shared postgres");
  ...
  test("block timeline renders with coverage and both sliders (E7)", async ({ page }) => {
    await page.goto(EXPLORER_WEB);
    const timeline = page.locator("#timeline");
    // the timeline unhides once the RPC head is known
    await expect(timeline).toBeVisible({ timeout: 30_000 });
    await expect(page.locator("#timelineHandle")).toBeVisible();
    await expect(page.locator("#timelineInterval")).toBeVisible();
    await expect(page.locator("#timelineStart")).toHaveText(/^#\d+/);
    await expect(page.locator("#timelineEnd")).toHaveText(/head/);

    // green coverage segments appear when inspected blocks fall in the window
    const block = latestInspectedBlock();
    const head = Number((await page.locator("#timelineEnd").textContent())?.match(/\d+/)?.[0]);
    if (block !== null && head - block < 5000) {
      await expect
        .poll(() => page.locator("#timelineCoverage .cov").count(), { timeout: 15_000 })
        .toBeGreaterThan(0);
    }
  });
});
